/* ========================================================================
 * Includes    : AdminController.cs
 * Website     : http://www.atlassoft.com
 * Create Date : May, May 08, 2019 by Sudipta Sarkar
 * Details     : Implements logics for tracking logs generated for AggieGlobal.WebApi.
...
//using AggieGlobal.WebApi.Providers;
...
//    {
//        [HttpGet]
//        public IEnumerable<WebApiLog> GetLogs(string type = "Request,Response,General", int last = 10, string ip = "", string source = "ado")
//        {
//            if (type.ToLower().StartsWith("clear"))
//            {
//                new DataRepository("DeviceManagerDB").ExecuteQuery("DELETE FROM MI_ServiceLog WHERE DATEDIFF(D, CreateDate, GetDate()) > " + type.ToLower().Replace("clear", string.Empty));
//                return new List<WebApiLog>();
//            }
//            else
//            {
//                return source.ToLower().Equals("ado") ? Log.GetAdoAppenderLogs(type, last, ip) : Log.GetFileAppenderLogs(type, last, ip);
//            }
//        }

//        [AcceptVerbs("GET", "POST")]
//        public HttpResponseMessage Tokens(string action)
//        {
//            return action.ToLower().Equals("get") ? new HttpResponseMessage() { Content = new StringContent(new DbTokenProvider().GetTokens().ToString()) } :
//                new HttpResponseMessage() { Content = new StringContent(new DbTokenProvider().LoadTokens(XDocument.Load(new StringReader(Request.Content.ReadAsStringAsync().Result))).ToString()) };
//        }

//        [HttpGet]
//        public HttpResponseMessage Current(string context)
//        {
//            var identity = WindowsIdentity.GetCurrent();
//            JObject identityObject = new JObject();
//            identityObject.Add("AuthenticationType", identity.AuthenticationType);
//            identityObject.Add("IsAuthenticated", identity.IsAuthenticated);
//            identityObject.Add("ImpersonationLevel", identity.ImpersonationLevel.ToString());
//            identityObject.Add("IsAnonymous", identity.IsAnonymous);
//            identityObject.Add("IsGuest", identity.IsGuest);
//            identityObject.Add("IsSystem", identity.IsSystem);
//            identityObject.Add("Name", identity.Name);

//            JObject processObject = new JObject();
//            using (var process = System.Diagnostics.Process.GetCurrentProcess())
//            {
//                processObject.Add("WorkingSet", process.WorkingSet64);
//                processObject.Add("PrivateBytes", process.PrivateMemorySize64);
//                processObject.Add("HandleCount", process.HandleCount);
//                processObject.Add("ThreadsCount", process.Threads.Count);
//                processObject.Add("ModulesCount", process.Modules.Count);
//                processObject.Add("MachineName", process.MachineName);
//                processObject.Add("ProcessId", process.Id);
//                processObject.Add("DeployedTime", new System.IO.FileInfo(System.Reflection.Assembly.GetAssembly(this.GetType()).Location).LastWriteTime);
//                processObject.Add("StartTime", process.StartTime);
//                processObject.Add("CurrentTime", DateTime.Now);
//            }

//            JArray returnObject = new JArray();
//            returnObject.Add(identityObject);
//            returnObject.Add(processObject);

//            return new HttpResponseMessage(HttpStatusCode.OK) { Content = new ObjectContent<JArray>(returnObject, FormatterConfig.JsonFormatter) };
//        }

//        [HttpPut]
//        public string PutRequestTest()
//        {
//            return "Successfull PUT response from API service";
//        }
//    }
